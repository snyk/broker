  ...
  io.on('connection', function (socket) {
    ...
    const close = (closeReason = 'none') => {
      if (token) {
        ...
        logger.info({ closeReason, maskedToken, remainingConnectionsCount: clientPool.length },
    ...
    socket.on('identify', clientData => {
      ...
      if (!token) {
        logger.warn({ token, metadata: clientData.metadata }, 'new client connection identified without a token');
        return;
      }

      const maskedToken = token.slice(0,4) + '-...-' + token.slice(-4);

      logger.info({ maskedToken, metadata: clientData.metadata }, 'new client connection identified');
