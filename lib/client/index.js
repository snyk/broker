...
const { executePreflightChecks, preflightChecksEnabled } = require('./checks');
const {
  handleChecksRoute,
  handleCheckIdsRoutes,
} = require('./checks/api/checks-handler');
const { commitSigningEnabled, commitSigningFilterRules } = require('./scm');
...
module.exports = async ({ port = null, config = {}, filters = {} }) => {
  ...
  if (preflightChecksEnabled(config)) {
    // wrap preflight checks execution into try-catch, so the broker client
    // will start anyway
    try {
      preflightCheckResults = await executePreflightChecks(config);
    } catch (error) {
  ...
  if (commitSigningEnabled(config)) {
    ...
    if (config.BROKER_BOOT_MODE == 'universal') {
      filters['github'].private?.push(...commitSigningRules);
      filters['github-enterprise'].github.private?.push(...commitSigningRules);
    } else {
      filters.private?.push(...commitSigningRules);
    }
    logger.info(
  ...
  const io = socket({
    ...
    filters: config.BROKER_BOOT_MODE == 'universal' ? filters : filters.private,
    config,
    identifyingMetadata,
    serverId,
  });
  ...
  app.get('/health/checks', handleChecksRoute(config));
  app.get('/health/checks/:checkId', handleCheckIdsRoutes(config));
  ...
  // Universal broker must have all webhook filters together since we don't know the type of system we get the webhook from.
  const filtersForRequest =
    config.BROKER_BOOT_MODE == 'universal'
      ? utils.concatAllPublicFiltersIntoArray(filters)
      : filters.public;
  ...
  app.all(
    ...
    relay.request(filtersForRequest),
  );
