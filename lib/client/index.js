...
const { getServerId, highAvailabilityModeEnabled, defaultBrokerDispatcherBaseUrl } = require('./dispatcher');
...
module.exports = async ({ port = null, config = {}, filters = {} }) => {
  ...
  const url = new URL(config.brokerServerUrl || defaultBrokerDispatcherBaseUrl);
  url.searchParams.append('server_id', serverId);
  const brokerServerUrl = url.toString();
...
// function repeatUntilSucceeds(config, token, brokerClientId) {
...
//       //getServerId(config, token, brokerClientId)
//       getServerId(config, token, brokerClientId)
//         .then((serverId) => {
...
//             resolve(serverId);
//           } else {
//             handleErrorRec(new Error(serverId));
//           }
//         })
