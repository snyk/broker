  ...
  return (brokerToken) => (
    ...
    filters({ url, method, body, headers }, (filterError, result) => {
      ...
      if (streamingID) {
        ...
        request(req)
          .on('response', (response) => {
            ...
            isResponseGzip = isGzip(response.headers);
          ...
          .on('data', (chunk) => {
            if (config.RES_BODY_URL_SUB && !isResponseGzip) {
      ...
      request(req, (error, response, responseBody) => {
        ...
        const status = (response && response.statusCode) || 500;
        if (config.RES_BODY_URL_SUB && !isGzip(response.headers)) {
...
function isGzip(responseHeaders) {
  return responseHeaders['content-type'] ? responseHeaders['content-type'].includes('gzip') : false;
}
