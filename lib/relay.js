...
function forwardHttpRequest(filterRules) {
  const filters = Filters(filterRules, 'public'); // public only here
  ...
  return (req, res) => {
    ...
    const logContext = {
      ...
      type: req.headers['x-snyk-broker-type'] || '',
    };
...
function forwardWebSocketRequest(rawFilterRules, config, io, serverId) {
  return (brokerToken) =>
    (
      { url, headers = {}, method, body = null, streamingID = '' } = {},
      emit,
    ) => {
      const targetedServiceType =
        config.BROKER_BOOT_MODE == 'universal' && headers['x-snyk-broker-type']
          ? utils.getRequestTypeFromTypeId(
              config,
              headers['x-snyk-broker-type'],
            )
          : null;
      const filterRules = targetedServiceType
        ? rawFilterRules[targetedServiceType].private
        : rawFilterRules;

      const filters = Filters(filterRules, 'private'); // expected private only here
