...
const { ResponseStreamer } = require('./stream-posts');
const { GitHubCache } = require('./gh-git-clone');
...
function forwardWebSocketRequest(filterRules, config, io) {
  const filters = Filters(filterRules);
    ...
    (
      ...
      emit = (responseData) => {
        ...
        if (io?.capabilities?.includes('receive-post-streams')) {
          ...
          if (
            responseData instanceof request.Request ||
            responseData instanceof Readable
          ) {
            logger.trace(
              logContext,
              'posting streaming response back to Broker Server',
            );
            responseStreamer.pipeStream(responseData);
          } else {
            // Only for responses generated internally in the Broker Client/Server
            const body = responseData.body;
            delete responseData.body;
            responseStreamer.sendData(responseData, body);
          }
        } else {
          if (streamingID) {
      ...
      filters({ url, method, body, headers }, (filterError, result) => {
        ...
        // headers.authorization is undefined if we're doing a git clone, for example, where it's part of the URL.
        if (GitHubCache.enabled() && headers.authorization) {
          logger.trace(logContext, 'github cache enabled');
          const cache = new GitHubCache(
            logContext,
            result.url,
            headers.authorization.substring(6),
          );
          if (cache.pathTriggersCaching()) {
            cache.loadCache();
          } else if (cache.pathSupported()) {
            if (
              cache.handle(
                (stream) => {
                  emit(stream);
                ...
                (code) => {
                  emit({
                    ...
                    body: {
                      code,
                      message:
                        'there was an error creating the cache via a GitClone',
                    },
                  });
                },
              )
              ...
              logger.trace(
                logContext,
                'cache not present - calling out to GitHub directly',
              );
            }
          } else {
            logger.trace(logContext, 'url not supported for clone cache');
          }
        }
