...
const needleRequest = require('needle').request;
  ...
  return (brokerToken) => ({ url, headers = {}, method, body = null, streamingID = '' } = {}, emit) => {
    ...
    filters({ url, method, body, headers }, (error, result) => {
      ...
      if (streamingID) {
        ...
        // TODO refactor to needle (use stream and subscribe to `readable` event)
      ...
      needleRequest(req.method, req.url, req.body, {
        headers: req.headers,
        ca: req.agentOptions.ca,
        'follow_max': 1, // http => https redirect,
      }, (error, response, body) => {
        if (error) {
          logError(logContext, error);
          return emit({
        ...
        // Don't pipe into socket gzip header, cos npm will try to
        // unzip it and fail, cos it's gonna be a valid JSON
        ...
        if (typeof body === 'object') {
          ...
          respBody = body.toString();
        }

        const status = (response && response.statusCode) || 500;
        logResponse(logContext, status, response.headers);
        emit({ status, body: respBody, headers: response.headers });
      });
    });
  };
}
