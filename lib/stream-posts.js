...
function initStream(config, brokerToken, streamingID, ioData) {
  ...
  buffer.pipe(
    request({
      ...
      .on('response', (r) => {
        if (r.statusCode !== 200) {
          logger.error('Received unexpected response uploading data', {
...
function pipeStream(rqst, logContext, config, brokerToken, streamingID) {
  ...
  rqst
    ...
    .on('response', (response) => {
      ...
      buffer = initStream(config, brokerToken, streamingID, ioData);
    })
    .on('data', (chunk) => {
