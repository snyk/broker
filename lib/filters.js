...
module.exports = (ruleSource, filterType) => {
  let rules = {};
  ...
  if (Array.isArray(ruleSource)) {
    rules['default'] = {};
    rules['default'][filterType] = ruleSource;
  } else if (ruleSource) {
    try {
      rules = ruleSource;
    } catch (error) {
  ...
  Object.keys(rules).forEach((key) => {
    if (!Array.isArray(rules[key][filterType])) {
      throw new Error(
  ...
  const testsForAllSystems = {};

  Object.keys(rules).forEach((ruleSystemType) => {
    const tests = [];
    const testsForKey = rules[ruleSystemType][filterType].map((entry) => {
      const keys = [];
      let {
        method,
        origin,
      ...
      return (req) => {
        const reqBrokerTypeId = req.headers
          ? req.headers['x-snyk-broker-type']
          : null;
        const reqSystemType = utils.getRequestTypeFromTypeId(
          config,
          reqBrokerTypeId,
        );
        if (
          ruleSystemType != 'default' &&
          reqSystemType != ruleSystemType // if no loaded rules match type, then block
        ) {
          logger.debug(
            { reqBrokerTypeId, reqSystemType },
            'no loaded rules match request type',
          );
          return false;
        }
    ...
    tests.push(...testsForKey);
    testsForAllSystems[ruleSystemType] = tests;
  });

  return (payload, callback, targetedServiceType = 'default') => {
    let res = false;
    logger.debug(
      {
        rulesCount: Object.keys(testsForAllSystems).length
          ? testsForAllSystems[
              targetedServiceType ? targetedServiceType : 'default'
            ].length
          : 0,
      },
    ...
    try {
      for (const test of testsForAllSystems[
        targetedServiceType ? targetedServiceType : 'default'
      ]) {
        res = test(payload);
        if (res) {
