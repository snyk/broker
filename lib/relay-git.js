...
function responseHandler(config, io) {
  return (brokerToken) => (
    ...
    if (body) {
      ...
      if (parsedBody && parsedBody.key && parsedBody.key.gitURI) {
        parsedBody.key.gitURI = parsedBody.key.gitURI.includes('://') ?
          // using schema as anchor to inject the git server url
          parsedBody.key.gitURI.replace(/:\/\/[^\/]+/, `://${GIT_URL}`) :
          // best effort approximation using https schema
          `https://${GIT_URL}${parsedBody.key.gitURI.startsWith('/') ? '' : '/'}${parsedBody.key.gitURI}`;
