...
function requestHandler(config) {
  return (req, res) => {
    ...
    if (!GIT_CLIENT_URL || !GIT_CLIENT_CREDENTIALS) {
      ...
      return res.status(500).json({ error: GIT_CLIENT_URL ? 'MISSING_GIT_CLIENT_CREDENTIALS' : 'MISSING_GIT_CLIENT_URL'});
    }
    ...
    if (req.body && req.url === '/analyze') {
      const parsedBody = tryJSONParse(req.body);
      if (parsedBody.url) {
        parsedBody.url = parsedBody.url.replace("://", `://${GIT_CLIENT_CREDENTIALS}@`);
