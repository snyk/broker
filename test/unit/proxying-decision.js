...
test('shouldProxy: no https proxy', (t) => {
  ...
  t.false(
    shouldProxy(parse('https://broker.snyk.io')),
...
test('shouldProxy: no_proxy', (t) => {
  ...
  t.true(shouldProxy(parse('https://shambhala.org/')), 'not in no_proxy');
});
...
test('shouldProxy: NO_PROXY', (t) => {
  ...
  t.false(
    shouldProxy(parse('https://' + dontProx + '/?XeroxParc')),
    'should not proxy domains from NO_PROXY',
  );
  t.true(shouldProxy(parse('http://silibank.net.kp')), 'not in NO_PROXY');
});
